...
class MeteorMovement : public Component
{
public:
    ...
    MeteorMovement(XMFLOAT2 d, float s = 0.5f)
    {
        dir = d;
        speed = s;
    }
    ...
    {
        // 이동
        pOwner->pos.x += dir.x * speed * dt;
        pOwner->pos.y += dir.y * speed * dt;

        // 화면 밖 삭제
        if (pOwner->pos.x < -1.5f ||
            pOwner->pos.x > 1.5f ||
            pOwner->pos.y < -1.5f ||
            pOwner->pos.y > 1.5f)
        {
...
public:
    ...
    MeteorSpawner(GameLoop* g, Mesh* mesh, Material* mat)
    {
        engine = g;
        meteorMesh = mesh;
    ...
    void SpawnMeteor()
    {
        ...
        GameObject* meteor =
            new GameObject(spawnX, spawnY, 0);

        meteor->scale = { 0.15f,0.15f,1 };
...
int WINAPI WinMain(HINSTANCE hI, HINSTANCE, LPSTR, int nS)
{
    ...
    std::vector<Vertex> vMeteor;

    for (int i = 0; i < 10; i++)
    {
        vMeteor.push_back({ {0,0,0}, {1,1,1,1} });

        vMeteor.push_back({ pMeteor[i], {1,1,1,1} });

        vMeteor.push_back({ pMeteor[(i + 1) % 10], {1,1,1,1} });
    }
    Mesh* meteorMesh = new Mesh();

    meteorMesh->vertexCount = vMeteor.size();

    meteorMesh->Create(&gEngine.gfx, vMeteor);
    ...
    gEngine.world.push_back(spawner);
    ...
    meteorSpawnerObject->AddComponent(
        new MeteorSpawner(
            &gEngine,
            gMesh,
            meteorMat));

    gEngine.world.push_back(meteorSpawnerObject);
